...
def call_create_dataset_rnn(df):
    df=df.drop([0,1])
    df=df.rename(columns={0:"TIME"})
    df["TIME"] = pd.to_datetime(df["TIME"])
    df=df.set_index("TIME")
...
#    print(len(data.index))
    ...
    return dataset_rnn,dataset_test,traning_step,sample_step,rc_step
...
#+++++++++++++++++++++++++++++++
DATASET,DATASET_test,TRANING_STEP,SAMPLE_STEP,RC_STEP = call_create_dataset_rnn(dataframe)
print(DATASET)
datalen  = DATASET.shape[0] #時間長さ
datalen2 = DATASET.shape[1] #入力＋出力長さ
#+++++++++++++++++++++++++++++++
...
DATA_ori = DATA_ori.reshape((RC_STEP,IN_NODE+OUT_NODE))
DATA_rc  = DATA_ori[:,0:IN_NODE]
...
#U_in = zscore(U_in,axis=0)
#S_out = zscore(S_out,axis=0)
#DATASET_test = zscore(DATASET_test,axis=0)

print(U_in.shape)
print(S_out.shape)
#print(DATA_rc.reshape(-1,2))
...
#            ,W_out)
#W_out1=W_out.T
#call_fortran_rnn_traning_own(IN_NODE,OUT_NODE,RNN_NODE,TRANING_STEP,RC_STEP,
...
call_fortran_rnn_traning_own(IN_NODE,OUT_NODE,RNN_NODE,SAMPLE_STEP,RC_STEP,
            SAMPLE_NUM,EPOCH,EPSILON,G
            ,U_in[0:TRANING_STEP,0:IN_NODE],S_out[0:TRANING_STEP,0:OUT_NODE]
            ,DATA_rc[:,0:IN_NODE],S_rc[:,0:OUT_NODE]
            ,W_out1,W_rnn,W_in,Tre_CH)
...
#print(DATA_rc.shape)
S_rc    = S_rc.reshape((RC_STEP,OUT_NODE))
...
#ax1.plot(DATA_rc[:,0:IN_NODE],"-" , label="rc")
