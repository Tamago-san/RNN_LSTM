subroutine rnn_traning_own_fortran(in_node,out_node,rnn_node,traning_step,rnn_step,&
    ...
    real(8)     s_tr_data(traning_step,out_node)  !出力次元数、列サイズはトレーニング時間
    ...
    !＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
    !転置
    call inverse_matrix(u_trT,u_tr,in_node,traning_step)
    call inverse_matrix(s_tr_dataT,s_tr_data,out_node,traning_step)
...
!    W_out=W_out/(rnn_node)**0.5
    ...
    do i=1,traning_step
        write(20,*) i,u_tr(i,in_node),s_tr_data(i,out_node)
    enddo
    do iepo=1,epoch
        if(mod(iepo,500)==0) write(*,*) "epoch === " ,iepo
        do isample=1,sample_num
            ...
            call rnn_forward(u_tr,s_tr,R_tr,z_tr,traning_step,rnn_node)
            
            !逆伝播計算
            do istep=traning_step,1,-1
                
                !W_out
                out_delta(istep,:)= (s_tr(istep,:) - s_tr_data(istep,:))
                do i=1,out_node
                do j=1,rnn_node
                ...
                do i=1,rnn_node
                do j=1,in_node
                    in_dEdw(i,j)=in_dEdw(i,j)+rnn_delta(istep,i) * u_tr(istep,j)
                enddo
                enddo
            ...
            do i=1,rnn_node
            do j=1,rnn_node
                W_rnn(j,i) =W_rnn(j,i) - 0.0001*rnn_dEdw(j,i)
            enddo
            enddo
