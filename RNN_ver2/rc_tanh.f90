...
subroutine rc_traning_own_fortran(in_node,out_node,rc_node,traning_step,rc_step,gusai,alpha,g,&
                    u_traningT,s_traningT,u_rcT,s_rcT,w_outT)
    implicit none
    integer(4), intent(inout) :: in_node,out_node,rc_node,traning_step,rc_step
    real(8),    intent(inout) :: gusai,alpha,g
    real(8),    intent(inout) :: w_outT(out_node,rc_node)
    real(8),    intent(inout) ::u_traningT(in_node,traning_step) !今は一次元、列サイズはトレーニング時間
    real(8),    intent(inout) ::s_traningT(out_node,traning_step)  !出力次元数、列サイズはトレーニング時間
    real(8),    intent(inout) ::u_rcT(in_node ,rc_step) !今は一次元、列サイズはトレーニング時間
    real(8),    intent(inout) ::s_rcT(out_node,rc_step)  !出力次元数、列サイズはトレーニング時間
    real(8)     u_traning(traning_step,in_node) !今は一次元、列サイズはトレーニング時間
    real(8)     s_traning(traning_step,out_node)  !出力次元数、列サイズはトレーニング時間
...
!    write(*,*) a(1:rc_node,1:rc_node)
    do i=1,rc_node
    do k=1,rc_node
        a(i,k)=rand_normal(0.d0, 1.d0/(rc_node**0.5d0))
    enddo
    enddo
